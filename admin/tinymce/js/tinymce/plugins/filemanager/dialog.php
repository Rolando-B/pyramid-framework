...
<!----- header div end ------->
	...
	$link.=$_GET['lang'] ? $_GET['lang'] : 'fr_FR';
	$link.="&field_id=";
		...
		foreach ($files as $file) {
			...
			<li>
				<figure>
				    ...
				    href="dialog.php?type=<?php echo $_GET['type']?>&subfolder=<?php echo $subfolder ?>&editor=<?php echo $_GET['editor'] ? $_GET['editor'] : 'mce_0'; ?>&popup=<?php echo $popup;?>&field_id=<?php echo $_GET['field_id'] ? $_GET['field_id'] : '';?>&lang=<?php echo $_GET['lang'] ? $_GET['lang'] : 'fr_FR'; ?>&fldr=<?php echo $src ?>&<?php echo uniqid() ?>">
			<?php if($file==".."){ ?>
			...
			if ($file != '.' && $file != '..' && !is_dir($root . $cur_dir . $file)) {
				...
				//add in thumbs folder if not exist 
				$thumb_path=dirname( __FILE__ ).DIRECTORY_SEPARATOR."thumbs".$subfolder.DIRECTORY_SEPARATOR.$subdir.$file;
